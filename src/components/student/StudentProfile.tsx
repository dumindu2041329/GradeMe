...
const StudentProfile = () => {
  ...
  useEffect(() => {
    try {
      const savedProfileData = localStorage.getItem('studentProfileData');
      if (savedProfileData) {
        ...
        const validatedData: ProfileData = {
          ...
          notifications: {
            email: {
              examResults: parsedData.notifications?.email?.examResults ?? defaultProfileData.notifications.email.examResults,
              upcomingExams: parsedData.notifications?.email?.upcomingExams ?? defaultProfileData.notifications.email.upcomingExams,
            },
            sms: {
              examResults: parsedData.notifications?.sms?.examResults ?? defaultProfileData.notifications.sms.examResults,
              upcomingExams: parsedData.notifications?.sms?.upcomingExams ?? defaultProfileData.notifications.sms.upcomingExams,
            },
          },
        };
  ...
  const handlePasswordChange = (e: React.FormEvent) => {
    ...
    const storedPassword = localStorage.getItem('studentPassword') || 'student123';
    if (passwordData.currentPassword !== storedPassword) {
      alert('Current password is incorrect');
      return;
    }

    localStorage.setItem('studentPassword', passwordData.newPassword);
  ...
  const handleProfileUpdate = (e: React.FormEvent) => {
    ...
    localStorage.setItem('studentProfileData', JSON.stringify(editData));
    localStorage.setItem('studentProfilePicture', editData.profilePicture);
    setActiveTab('view');
  ...
  const handleNotificationChange = (type: 'email' | 'sms', setting: 'examResults' | 'upcomingExams') => {
    const newData = {
      ...editData,
      notifications: {
    ...
    localStorage.setItem('studentProfileData', JSON.stringify(newData));
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="h-[calc(100vh-72px)] overflow-y-auto px-4 lg:px-0 pt-[88px]">
                          ...
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              ...
                              <Toggle
                                ...
                                onChange={() => handleNotificationChange('email', 'examResults')}
                              />
                            </div>
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="text-sm text-card-foreground">Upcoming Exams</p>
                                <p className="text-sm text-muted-foreground">Get notified about upcoming exams</p>
                              </div>
                              <Toggle
                                checked={editData.notifications.email.upcomingExams}
                                onChange={() => handleNotificationChange('email', 'upcomingExams')}
                              />
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              ...
                              <Toggle
                                ...
                                onChange={() => handleNotificationChange('sms', 'examResults')}
                              />
                            </div>
                            <div className="flex items-center justify-between">
                              <div>
                              ...
                              <Toggle
                                checked={editData.notifications.sms.upcomingExams}
                                onChange={() => handleNotificationChange('sms', 'upcomingExams')}
                              />
                            </div>
                          </div>
                        </div>
